<script>
	...
	// 	];
			let steps = 
				...
				"As outlined by a cartogram approach, we consider each of these sub-indices and, ultimately, our overall Danger Index at the state-level.", 
				 ...
				 "For example, it can be observed that both Alaska (42%) and Washington D.C. (42%) provide the most reproductive health services to residents.  Contrastingly, Texas (94%) lacks the most in reproductive health services for residents.",
				 ...
				 "Here, we can see that women are subjected to the least amount of violent crime in the state of New York (2%).  In Arkansas (100%), however, women are subjected to the most amount of violent crime.", 
				 ...
				 "But what do politics have to do with it?<br><br>Our analysis illustrates that red states (i.e., states whose voters predominantly choose the Republican Party) are consistently more dangerous than blue states (i.e., states whose voters predominantly choose the Democratic Party) for female assigned at birth (AFAB) and femme folx.  In fact, our overall danger index indicates that, on average, red states are 80% more dangerous than blue states for these people.",
	...
	$: statesIndex = states.map(d=>({...d, 
						titleXNorm: titlexNormScale(d.titlexNum/d.femalePop),
						famPlanCentersNorm: famPlanCentersNormScale(d.famPlanCentersNum/d.femalePop),
						aProvidersNorm: aProvidersNormScale(d.aProvidersNum/d.femalePop),
						famPlanCenterSpendNorm: FamPlanCenterNormScale(d.totalPubDollarsFamPlanCenter/d.femalePop),
						abortionDollarsNorm: AbortionDollarsNormScale(d.totalPubDollarsAbortion/d.femalePop),
